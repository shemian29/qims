...
class pxp_model():
    ...
    def __init__(self, size, basis, basis_ind):
        self.size = size
        self.basis = basis
        self.basis_ind = basis_ind

    # bs, bs_ind = qims.basis(self.size, parallel=False)

    def pxp_hamiltonian(self):
        """
        PXP Hamiltonian
        ...
        """
        h = 0
        # Run over multi-site operators in the Hamiltonian labeled by each site position 1<=r<=size
        for r in tqdm.notebook.tqdm(range(self.size)):
            scan = []
            for st in range(len(self.basis)):

                if constr(self.basis[st], r, self.size) == 1:
                    scan.append([self.basis_ind[pxp(self.basis[st], r, self.size)], st, 1])
            ...
            col = elms.T[1]
            data = elms.T[2]
            h = h + scipy.sparse.csr_matrix((data, (row, col)), shape=(len(self.basis), len(self.basis)))
