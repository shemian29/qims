...
def Op(operator, N, r):
    """
    Represent operator in the full Hilbert space
    :param operator: Operator
    :param N: System size
    :param r: Location of operator
    :return:
    """
    ops = []
    Id = qt.identity(operator.shape[0])
...
def ind2occ(s, r, N, Ncut):
    return (s // ((2 * Ncut + 1) ** (N - r - 1))) % (2 * Ncut + 1)
...
def GenerateMomentumBasis(N, Ncut):
    ...
    while len(lst) > 0:
        ...
        np.savetxt("Monitor_" + str([N, Ncut]) + ".txt", (100 * len(lst) / dms),
                   fmt='%1.4f')
