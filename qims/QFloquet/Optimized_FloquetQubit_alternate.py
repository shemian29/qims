...
class FloquetQubit_alt:
    ...
    def search_floquet_qubit(self, number_frequencies: int = 2, maxiter: int = 10000000) -> scp.optimize.OptimizeResult:
        ...
        try:
            self.cost0 = self.cost_function([self.E01, 12.3*self.E01 , self.φ_0, self.θ_0, self.β_0], normalize=False) #The choice of Floquet frequency is arbitrary since it does not affect the static system
        except:
    ...
    def cost_function(self, parameters_extended: np.ndarray, hyper_parameter: float = 100, normalize= True) -> float:
        ...
        self.floquet_qubit_parameters['dynamic']['ε01'] = parameters_extended[0].real
        self.floquet_qubit_parameters['dynamic']['νfloquet'] = parameters_extended[1].real

        frequency_components = self.complexify(parameters_extended)
        self.floquet_qubit_parameters['dynamic']['φ_m'] = frequency_components[0]
        self.floquet_qubit_parameters['dynamic']['θ_m'] = frequency_components[1]
        self.floquet_qubit_parameters['dynamic']['β_m'] = frequency_components[2]


        decoh_rate = hyper_parameter * self.decoherence_rate()

        if normalize:
            return (self.hstatic_matching() / decoh_rate + decoh_rate) / self.cost0
        else:
            return self.hstatic_matching() / decoh_rate + decoh_rate
    def record_differential_optimization_path(self, parameters, convergence=1):
        ...
        iteration_step += 1
    def decoherence_rate(self, parameters = self.floquet_qubit_parameters) -> float:
        """
    ...
    def hstatic_matching(self) -> float:
        """
    ...
    def su2_rotation_dot(self, φ: float, θ: float, β: float, φ_dot: float, θ_dot: float, β_dot: float) -> qt.Qobj:
        ...
        return sm
    def su2_rotation_freq_to_time(self, parameters = self.floquet_qubit_parameters) -> List[qt.Qobj]:
        """
    ...
    def su2_rotation_freq_to_time_dot(self, parameters = self.floquet_qubit_parameters) -> List[qt.Qobj]:
        """
